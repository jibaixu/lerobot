                    ...
                    if feature in df.columns:
                        ...
                        for _idx, value in enumerate(df[feature]):
                            if value is not None and isinstance(value, (list, np.ndarray)):
                                ...
                                max_dima = 7
                                if current_dim < max_dima:
                                    print(
                                        f"填充 {feature} 从 {current_dim} 维到 {max_dima} 维 (Padding {feature} from {current_dim} to {max_dima} dimensions)"
                                    )
                                    # 使用零填充到目标维度 (Pad with zeros to target dimension)
                                    df[feature] = df[feature].apply(
                                        lambda x: np.pad(x, (0, max_dima - len(x)), "constant").tolist()
                                        if x is not None
...
def merge_datasets(
    source_folders, output_folder, validate_ts=False, tolerance_s=1e-4, max_dim=8, default_fps=10
):
    """
    ...
    for folder in source_folders:
        try:
            ...
            folder_dim = max_dim  # 使用变量替代硬编码的8
            ...
            for episode in episodes:
                ...
                if old_index in stats_map:
                    ...
                    if "stats" in stats and folder_dimensions[folder] < max_dim:  # 使用变量替代硬编码的18
                        # Pad observation.state and action stats
                        for feature in ["observation.state"]:
                            if feature in stats["stats"]:
                                for stat_type in ["mean", "std", "max", "min"]:
                                    if stat_type in stats["stats"][feature]:
                                        # Get current values
    ...
    if "features" in info:
        ...
        for feature_name in ["observation.state"]:
        ...
        actual_action_dim = 7
        for feature_name in ["action"]:
            if feature_name in info["features"] and "shape" in info["features"][feature_name]:
                info["features"][feature_name]["shape"] = [actual_action_dim]
                print(f"Updated {feature_name} shape to {actual_action_dim}")
...
if __name__ == "__main__":
    source_folders = [
        "/data1_ssd/jibaixu/datasets/AllTasks-v4/panda_wristcam",
        "/data1_ssd/jibaixu/datasets/AllTasks-v4/widowxai_wristcam",
        "/data1_ssd/jibaixu/datasets/AllTasks-v4/xarm6_robotiq_wristcam",
        "/data1_ssd/jibaixu/datasets/AllTasks-v4/xarm7_robotiq_wristcam"
    ]
    output_folder = "/data1_ssd/jibaixu/datasets/AllTaskv4_unconvert"
    merge_datasets(source_folders, output_folder)

    # source_folders = ["/capacity/userdata/vcx5v9vr64h1/vla/maniskill/AllTasks-v4/panda_wristcam"]
    # output_folder = "/reasoning/xuehao/pi0/openpi/datasets/AllTaskv4_unconvert_panda"
    # merge_datasets(source_folders,output_folder)

    # source_folders = ["/capacity/userdata/vcx5v9vr64h1/vla/maniskill/AllTasks-v4/widowxai_wristcam"]
    # output_folder = "/reasoning/xuehao/pi0/openpi/datasets/AllTaskv4_unconvert_widowxai"
    # merge_datasets(source_folders,output_folder)

    # source_folders = ["/capacity/userdata/vcx5v9vr64h1/vla/maniskill/AllTasks-v4/xarm6_robotiq_wristcam"]
    # output_folder = "/reasoning/xuehao/pi0/openpi/datasets/AllTaskv4_unconvert_xarm6"
    # merge_datasets(source_folders,output_folder)

    # source_folders = ["/capacity/userdata/vcx5v9vr64h1/vla/maniskill/AllTasks-v4/xarm7_robotiq_wristcam"]
    # output_folder = "/reasoning/xuehao/pi0/openpi/datasets/AllTaskv4_unconvert_xamr7"
    # merge_datasets(source_folders,output_folder)
